    ...
    public void attack(Hero[] heroes) {
        for (Hero hero : heroes) {
            if (hero.getHealth() > 0) {
                ...
                if (hero instanceof Avrora && ((Avrora) hero).isInvisible()) {
                    System.out.println(hero.getName() + " avoided the damage due to invisibility!");
                    ((Avrora) hero).storeDamage(this.getDamage()); // Накопление урона
                    continue;
                }
